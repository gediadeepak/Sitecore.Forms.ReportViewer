...
namespace Sitecore.Forms.Report.Viewer.DataProvider
{
    public class SqlDataProvider : ISqlDataProvider
    {
        ...
        private string GenerateQuery(FormReportRequestModel request)
        {
            ...
            sb.Append(" Declare @SQLSTRING as nvarchar(max); SET @FORMITEMID = '" + request.FormId + "'; SET @STARTDATE = '" + request.StartDate.ToString("yyyy-MM-dd hh:mm:ss") + "'; SET @ENDDATE = '" + request.EndDate.ToString("yyyy-MM-dd hh:mm:ss") + "';set @ENDDATE=dateAdd(Hour,23,@ENDDATE);");
